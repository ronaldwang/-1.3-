...
- (void)_commonInit
{
    ...
    //  时间 标签
    self.dateLable = [[UILabel  alloc ] initWithFrame:CGRectMake(self.imageViewProgress.frame.size.width/2 - 100, self.imageViewProgress.frame.size.height/2 + 15, 200, 25)];
    self.dateLable.textAlignment = NSTextAlignmentCenter;
    self.dateLable.font =[UIFont fontWithName:@"HelveticaNeue-Light" size:12.0];
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset
{
    ...
    self.dateLable.text = [self.scrollView  takeRequestTimeDifference:[self.scrollView  takeRequestDate]];
    self.dateLable.textColor = [self.scrollView  colorWithHexString:[self.scrollView  takeColorString]];
...
-(void)actionTriggeredState
{
    ...
    [UIView animateWithDuration:0.00 delay:0.0 options:UIViewAnimationOptionCurveEaseInOut|UIViewAnimationOptionAllowUserInteraction animations:^{
