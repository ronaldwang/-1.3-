...
- (void)commonInit {
    ...
    _popUpAttribueColor = [UIColor  redColor];
...
- (void)layoutTouchReport {
    ...
    if (self.enableTouchReport == YES || self.enablePopUpReport == YES) {
        ...
        if (self.enablePopUpReport == YES && self.alwaysDisplayPopUpLabels == NO) {
            ...
            NSString *maxValueString = [NSString stringWithFormat:@"%.4f",
                                        [self calculateMaximumPointValue].floatValue];
            NSString *minValueString = [NSString stringWithFormat:@"%.4f",
            ...
            if ([self.delegate respondsToSelector:@selector(popUpSuffixForlineGraph:)])
                popUpString = [NSString stringWithFormat:@"%@%@",[self.delegate popUpSuffixForlineGraph:self],longestString];
            else popUpString = longestString;
...
- (void)drawEntireGraph {
    ...
    if (self.enableYAxisLabel) {
        ...
        if (self.autoScaleYAxis == YES){
            NSString *maxValueString = [NSString stringWithFormat:@"%.4f", (float)self.maxValue];
            NSString *minValueString = [NSString stringWithFormat:@"%.4f", (float)self.minValue];
            ...
            NSString *longestString = [NSString stringWithFormat:@"%.4f", (float)self.frame.size.height];
...
- (void)drawYAxis {
    ...
    if (self.autoScaleYAxis) {
        ...
        for (NSNumber *dotValue in dotValues) {
            ...
            labelYAxis.text = [NSString stringWithFormat:@"%.4f",[dotValue  floatValue]];
        ...
        for (NSInteger i = numberOfLabels; i > 0; i--) {
            ...
            labelYAxis.center = CGPointMake(self.YAxisLabelXOffset/2, yAxisPosition);
            labelYAxis.text = [NSString stringWithFormat:@"%.4f", (float)(graphHeight - self.XAxisLabelYOffset - yAxisPosition)];
            labelYAxis.font = self.labelFont;
            labelYAxis.textAlignment = NSTextAlignmentRight;
            labelYAxis.textColor = self.colorYaxisLabel;
            labelYAxis.backgroundColor = [UIColor clearColor];
...
- (void)displayPermanentLabelForPoint:(BEMCircle *)circleDot {
    ...
    if ([self.delegate respondsToSelector:@selector(popUpSuffixForlineGraph:)])
        permanentPopUpLabel.text = [NSString stringWithFormat:@"%@ %@",[self.delegate popUpSuffixForlineGraph:self], @(circleDot.absoluteValue)];
    else
        permanentPopUpLabel.text = [NSString stringWithFormat:@"%@", @(circleDot.absoluteValue)];
...
//    self.yCenterLabel = closestDot.center.y - closestDot.frame.size.height/2 - 15;
    ...
    if ([self.delegate respondsToSelector:@selector(popUpSuffixForlineGraph:)])
    {
        NSString  *popUpStirng = [NSString stringWithFormat:@"%@ %.4f", [self.delegate popUpSuffixForlineGraph:self],(float)[dataPoints[(NSInteger) closestDot.tag - DotFirstTag100] floatValue]];
        
        NSString  *suffixString = [self.delegate  popUpSuffixForlineGraph:self];
    
        NSMutableAttributedString  *attrtext = [[NSMutableAttributedString  alloc ] initWithString:popUpStirng];
    
          [attrtext addAttribute:(NSString *)NSForegroundColorAttributeName
                     value:self.popUpAttribueColor
